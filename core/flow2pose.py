...
def Flow2Pose(flow_up, depth, calib, return_medium=False, x=28, y=140):
    ...
    pred_depth_img += 1000.
    # 'out' is new uv index of original uv index, 'pred_depth_img' is new depth map
    output = visibility.image_warp_index(depth.to(device).contiguous(), flow_up.int(),
                                         pred_depth_img, output,
