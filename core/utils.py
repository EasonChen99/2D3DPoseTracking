...
def recover_rgb(img):
    ...
    if img.shape[0] == 1:
        cur_frame = img[0, ...].clone().cpu().permute(1, 2, 0).numpy()
        cur_frame = cur_frame * std + mean
        cur_frame = torch.tensor(cur_frame).float().permute(2, 0, 1).unsqueeze(0).to(img.device) * 255
    else:
        cur_frame = torch.zeros([img.shape[0], img.shape[1], img.shape[2], img.shape[3]])
        for i in range(img.shape[0]):
            temp = img[i, ...].clone().cpu().permute(1, 2, 0).numpy()
            temp = temp * std + mean
            cur_frame[i, ...] = torch.tensor(temp).float().permute(2, 0, 1).to(img.device) * 255
    return cur_frame